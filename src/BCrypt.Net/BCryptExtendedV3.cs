...
public sealed class BCryptExtendedV3 : BCryptCore
{
    ...
    {
        return CreatePasswordHash(inputKey, GenerateSalt(workFactor), hashType, (key, type, version, dest) => EnhancedHash(hmacKey, key, type, dest, version));
    }
    ...
    public static string HashPassword(string hmacKey, string inputKey, string salt, HashType hashType = DefaultEnhancedHashType) =>
        CreatePasswordHash(inputKey, salt, hashType, (key, type, version, dest) => EnhancedHash(hmacKey, key, type, dest, version));
    ...
    {
        return CreatePasswordHash(inputKey, salt, hashType, (key, type, version, dest) => EnhancedHash(hmacKey, key, type, dest, version));
    }
    ...
    private static int EnhancedHash(ReadOnlySpan<char> hmacKey, ReadOnlySpan<char> inputKey, HashType hashType, Span<byte> destination, char bcryptMinorRevision = 'a')
    {
        ...
        // UTF8 encode final base64 result into destination
        int utf8Len = SafeUTF8.GetBytes(finalBase64, destination);
    ...
    public static bool Verify(string hmacKey, string inputKey, string hash, HashType hashType = DefaultEnhancedHashType)
    {
        ...
        CreatePasswordHash(inputKey, hash, computedHash, out int computedHashLen, hashType,
            (key, type, version, dest) => EnhancedHash(hmacKey, key, type, dest, version));
